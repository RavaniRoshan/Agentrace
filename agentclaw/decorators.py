...
def _load_costs() -> dict:
    """Load model costs from ~/.agentclaw/costs.json."""
    costs_file = Path.home() / ".agentclaw" / "costs.json"
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                TraceStorage.save(t)
                print(f"\n[AgentClaw] Run complete -> {t.status.upper()}")
                print(
                    f"[AgentClaw] {len(t.steps)} steps | "
                    f"{t.total_tokens_in + t.total_tokens_out} tokens | "
                    f"${t.total_cost_usd:.4f} | "
                    f"{t.total_duration_ms}ms"
                )
                print(f"[AgentClaw] View trace: http://localhost:7823/trace/{t.trace_id}")
                if auto_open:
