...
## Output File
Traces are saved as JSON files in `~/.agentclaw/traces/<trace_id>.json` where `<trace_id>` is a unique identifier for each run.

## Terminal Output Format
When a traced function completes, AgentTrace prints a summary to the terminal:
```
