...
When an agent run fails, AgentTrace automatically analyzes the trace to identify the root cause. No configuration required — it fires on every failed run.
...
```python
from agentclaw import trace, trace_tool
