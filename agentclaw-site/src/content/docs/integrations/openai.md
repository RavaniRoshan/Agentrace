---
title: OpenAI
description: Using AgentTrace with OpenAI SDK.
---
...
```python
from agentclaw import trace_llm
import openai
...
AgentTrace automatically extracts token usage from OpenAI's response.usage object:
...
AgentTrace calculates costs based on OpenAI's official pricing:
...
AgentTrace uses these rates for cost estimation:
- **gpt-3.5-turbo**: $0.0005 per 1K input tokens, $0.0015 per 1K output tokens
- **gpt-4**: $0.03 per 1K input tokens, $0.06 per 1K output tokens
- **gpt-4-turbo**: $0.01 per 1K input tokens, $0.03 per 1K output tokens

## Note: Streaming Not Yet Supported
Currently, AgentTrace does not support tracing OpenAI streaming responses (`stream=True`). For streaming use cases:
...
## Error Handling
When tracing OpenAI calls, AgentTrace captures:
