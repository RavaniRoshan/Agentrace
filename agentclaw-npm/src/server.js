...
export const TRACES_DIR = join(homedir(), ".agentclaw", "traces");
...
import sys, json
from agentclaw.intelligence import analyse_trace
...
`], { env: process.env });
  ...
  const serveUI = (req, res) => {
    ...
    res.type("html").send("<h1>AgentClaw UI not found</h1>");
  };
...
export function migrateLegacyTraces() {
      const legacyDir = join(homedir(), '.agentclaw', 'traces');
  ...
  if (migrated > 0) console.log(`[AgentClaw] Migrated ${migrated} legacy traces to dashboard`);
}
